...
def test_add_sku_historic_sales_same_day_of_the_week():
    data = {'sku': [
                "1", "1", "2", "2", "2", "2",
                "2", "4", "4", "4", "4", "4"
            ],
    ...
    df = add_sku_historic_sales_same_day_of_the_week(df)
    assert \
        df[
            (df['sku'] == '1') &
            (df['warehouse'] == 'LASCONDES') &
...
def test_add_sku_historic_sales_same_month():
    ...
    df = add_sku_historic_sales_same_month(df)
    assert \
        df[
            (df['sku'] == '1') &
            (df['warehouse'] == 'COSTANERA') &
